...
public class touch1 : MonoBehaviour {
	...
	public void Start () {
		...
		//	textovalores.text="posinicialtouch:"+imagenboton.rectTransform.position.x+","+imagenboton.rectTransform.position.y;
			...
			imagenboton.rectTransform.position=new Vector2(imagenboton.rectTransform.position.x,imagenbotonPOSBASE.rectTransform.position.y);
			texto.rectTransform.position= new Vector2(texto.rectTransform.position.x,imagenboton.rectTransform.position.y);
	...
	void Update () {
		if(imagenboton.rectTransform.position.y>imagenbotonPOSBASE.rectTransform.position.y){
				imagenboton.rectTransform.position=new Vector2(imagenboton.rectTransform.position.x,imagenbotonPOSBASE.rectTransform.position.y);
			//imagenboton.rectTransform.position=new Vector2(imagenboton.rectTransform.position.x,imagenbotonPOSBASE.rectTransform.position.y);
			texto.rectTransform.position= new Vector2(texto.rectTransform.position.x,imagenboton.rectTransform.position.y);
		...
		//	}
			...
			imagenboton.rectTransform.position=new Vector2(imagenboton.rectTransform.position.x,touch2.y);
			texto.rectTransform.position= new Vector2(texto.rectTransform.position.x,imagenboton.rectTransform.position.y);
			/* 
			...
			if(tocando==true){
				if(actualY<centroY){direccion="abajo";}else{if(
					actualY>centroY){direccion="abajo";}
				}

				float textoyPosition=texto.rectTransform.position.y;
				
				//que le sume o le reste a textoyposition y luego lo aplique en su rectransform
				if(direccion=="abajo"){
					texto.text="abajo";
					textoyPosition+=1f;
						texto.rectTransform.position=new Vector2(texto.rectTransform.position.x,textoyPosition);
					//	texto.transform.Translate(0,1f,0);
					}
				if(direccion=="arriba"){
					texto.text="arriba";
					textoyPosition+=1f;
...
*/
			//imagenbotonPOSBASE.rectTransform.position=new Vector2(imagenbotonPOSBASE.rectTransform.position.x,imagenboton.rectTransform.position.y);
